...
start_link() ->
    lager:info("Starting OCCI table manager~n", []),
...
handle_info({'ETS-TRANSFER', TableId, OldOwner, _Data}, State) ->
    lager:debug("Receive 'ETS-TRANSFER' for ~p from ~p~n", [TableId, OldOwner]),
    {noreply, State};

handle_info({'EXIT', From, Reason}, State) ->
    lager:debug("Table owner died ~p: ~p~n", [From, Reason]),
    {noreply, State};
